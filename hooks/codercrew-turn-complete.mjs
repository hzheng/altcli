...
import { claudeCompletion, codexCompletion, contextKey, marker, plain } from './protocol.mjs';
...
async function main() {
  ...
  if (payload.hook_event_name === 'UserPromptSubmit') {
    // Two starts without an intervening Stop cannot be safely paired with one completion.
    // Fail closed rather than let a queued prompt borrow the preceding turn's final response.
    const previous = load();
    const commandId = previous?.phase === 'active' ? undefined : marker(payload.prompt);
    const context = { source: 'claude', paneId: target.paneId, socketPath: target.socketPath, identity: target,
      sessionId: payload.session_id, sourceTurnId: randomUUID(), prompt: plain(payload.prompt), ...(commandId ? { commandId } : {}) };
    save({ phase: 'active', context });
    await post({ ...context, event: 'turn_started' });
  } else if (payload.hook_event_name === 'Stop') {
    const saved = load();
    if (saved?.phase !== 'active' || !saved.context) return;
    const context = saved.context;
    if (context.sessionId !== payload.session_id || contextKey(context.identity, context.sessionId) !== contextKey(target, payload.session_id)) return;
    save({ phase: 'finished', context });
