...
The controlling input is the user-uploaded revised `Collaboration-Design.md` (mounted as `Collaboration-Design(1).md`), SHA-256 `d8430089eeddc5d2836b6985004a8f039e65fc48e0ff6c0a30a49bc45afe9f10`. Its 26 sections, 50 decision records, and 115 tabulated acceptance scenarios are redistributed into maintained documents; see [DESIGN-MIGRATION](DESIGN-MIGRATION.md). The input is not a tracked file in the inspected `46f228b` main tree, so no deletion of a nonexistent repository file is claimed and no duplicate monolithic draft is added. Earlier file names below identify provenance only.
