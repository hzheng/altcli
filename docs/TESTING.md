...
| Confirmed squash | The compact confirmation (consent digest plus a message of at most 8 KiB once JSON-encoded) fits the request limit: a 101-commit preview confirms with its unedited generated default, and a worst-case message of double quotes at exactly the budget round-trips through the body parser and commits; one more escaped byte is refused by the same rule the browser displays. A changed digest is refused before any claim. Persist the owner and the re-derived consent (edited message included), revalidate Git and ownership, then run one `git merge --squash` plus one `git commit` under the repository's normal hook policy; success is exactly one new single-parent commit with the previewed tree. A rejecting hook leaves a staged squash and an uncertain owner that inspection releases only after the checkout is restored; the task branch and worktree are unchanged and Check removal then verifies the squash. |
