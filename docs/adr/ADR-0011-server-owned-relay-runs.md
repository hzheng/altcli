...
UserPromptSubmit hook records a source-turn UUID for the specific command and sends
an acknowledgment. Stop uses only `last_assistant_message` for that response, never
an unanchored transcript fallback. Overlapping starts fail closed. Codex notify
uses its current prompt, thread ID and turn ID when available, with the unique
