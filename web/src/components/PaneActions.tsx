...
export function PaneActions(p: PaneActionsProps) {
  ...
  const [changesOpen, setChangesOpen] = useRemembered(`${p.draftKey}:changesOpen`, false);
  ...
  const snapshot = useReviewRange({ token: p.token, group, git, logPath: s.logPath, recipient: peerId, reviewTaskBase: s.reviewTaskBase, commitPending: true,
    enabled: canSend && pair && dirty, recheck: p.recheck, memoryKey: `${p.draftKey}:snapshot`, onPreview: revoke });
  ...
  const key = JSON.stringify(['pane', agent.id, agent.registrationId, group.id, group.revision, registrations, instances, p.agentsKey, git ?? null, p.workspaceError,
    s.consent, after, snapshot.consent, review.consent, p.runMark, p.recheck]);
  keyRef.current = key;
  ...
  function reasonFor(action: PaneAction): string {
    ...
    if (sending && !text.trim()) return `Enter an instruction for ${name}.`;
  ...
  async function start(action: PaneAction) {
    ...
    const request = paneRequest({ action, requestId, groupId: group.id, groupRevision: group.revision, registrations, agentId: agent.id, policy: s.selectedPolicy, workerId: s.workerId,
      text: action === 'commit' || action === 'commit_relay' ? note : action === 'relay' ? context : text, branch: s.branch(), automatic: s.automatic, turnLimit: s.limit,
    ...
    await p.submit(async () => {
      try {
        ...
        if (record.status === 'rejected') setStartError(`REJECTED: ${record.error ?? 'The server refused this start.'}`);
          ...
          if (action === 'commit' || action === 'commit_relay') { setNote(''); snapshot.reset(); } else if (action === 'relay') { setContext(''); review.reset(); } else setText('');
        }
      } catch (error) {
  ...
  const ids = useId();
  const sendChoice = sendAction(after); const sendReason = reasonFor(sendChoice);
  const sendLabel = after === 'commit' ? `Send & commit ${name}` : after === 'commit_relay' ? `Send & commit ${name} → relay ${peerName}` : `Send ${name}`;
  ...
  const sendHelp = after === 'nothing' ? `Send one instruction to ${name}. No automatic commit, branch change, or relay; committing requires an explicit instruction.`
  ...
  const authorization = after === 'nothing' ? `${name} carries out this instruction only. No commit, branch change or relay.`
  ...
  const commitHelp = `Commit ${name}: snapshot all staged, unstaged and nonignored untracked changes as they stand. No new implementation or automatic relay. One local handoff commit with Git hooks disabled; project checks still run.`;
  const snapshotHelp = `${name} snapshots all staged, unstaged and nonignored untracked changes as they stand, then the controller relays the selected baseline through the new commit to ${peerName} after validating publication and completion. No pending requests are implemented. One local handoff commit with Git hooks disabled; project checks still run.`;
  ...
  return <section className="pane-actions" aria-label={`Actions for ${name}`}>
    {canSend && <>
      ...
      <textarea id={`${ids}-text`} rows={2} value={text} disabled={inputOff} maxLength={1900} placeholder={`Instruction for ${name}`} onChange={(e) => setText(e.target.value)} />
      <div className="after-send"><label htmlFor={`${ids}-after`}>After send</label>
        <select id={`${ids}-after`} value={after} disabled={p.busy} onChange={(e) => setAfter(e.target.value as AfterSend)}>
          <option value="nothing">Nothing</option><option value="commit">Commit</option>{pair && <option value="commit_relay">Commit &amp; relay → {peerName}</option>}</select>
    ...
    {canSend && <p className="fine pane-line" id={`${ids}-line`}>{sendReason && sendReason !== NOT_READY ? sendReason : authorization}
      ...
      {sendReason && sendReason === s.branchReason && <button type="button" className="quiet inline-link" onClick={openSettings}>Edit settings</button>}</p>}
    {startError && <p className="notice error" role="alert">{startError}</p>}
    {canSend && dirty && <details className="pane-disclosure" open={changesOpen} onToggle={(e) => setChangesOpen(e.currentTarget.open)}>
      <summary>Current changes ({git!.changeCount} uncommitted)</summary>
      <p className="fine">Commits the changes as they stand; the note is context and is not implemented. On {branchLabel}: Commit reaches {name} only{pair ? `; Commit & relay reaches ${name}, then ${peerName}` : ''}.</p>
      <label htmlFor={`${ids}-note`}>Handoff note for {name} (optional)</label>
      <textarea id={`${ids}-note`} rows={2} value={note} disabled={inputOff} maxLength={1900} onChange={(e) => setNote(e.target.value)} />
      <div className="pane-buttons">
        <button type="button" title={title(reasonFor('commit'), commitHelp)} disabled={!!reasonFor('commit')} onClick={() => void start('commit')}>Commit {name}</button>
        {pair && <button type="button" title={title(reasonFor('commit_relay'), snapshotHelp)} disabled={!!reasonFor('commit_relay')} onClick={() => void start('commit_relay')}>Commit current changes &amp; relay {peerName}</button>}
      </div>
      {pair && <><RangePicker range={snapshot} recipientName={peerName} snapshot disabled={inputOff} inputDisabled={inputOff} />
        <p className="fine">{name} commits the current changes; {peerName} reviews all changes after the selected baseline through the new snapshot. Choose current HEAD to review only the current changes.</p></>}
    </details>}
    ...
    <details className="pane-disclosure authorizes"><summary>What each action authorizes</summary>
      <p className="fine">Readiness means every selected and unselected agent sharing this checkout is settled: prompts are empty and no background writers remain. Send authorizes only its instruction; with a Commit follow-up it also authorizes one handoff commit on the displayed branch choice, and with Commit &amp; relay one review of that commit by the named peer. Commit authorizes one snapshot of all staged, unstaged and nonignored untracked changes as they stand, without completing pending requests. Commit current changes &amp; relay also authorizes review from the selected baseline through that snapshot. Relay authorizes review of the displayed range and scoped handoff commits. A completed chain is not final task acceptance.</p>
    </details>
  </section>;
}
