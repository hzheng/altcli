...
const SETTINGS = ["ALTCLI_ADAPTER", "ALTCLI_ENABLE_INPUT", "ALTCLI_ENABLE_LEGACY_RELAY", "ALTCLI_DATA_DIR", "ALTCLI_TMUX_BIN", "ALTCLI_TMUX_SOCKET",
...
export function describeConfig(config: Config, env: Record<string, string | undefined> = process.env): HostConfig {
  return { mode: config.mode, inputEnabled: config.inputEnabled, legacyEnabled: config.legacyEnabled === true, dataDir: config.dataDir, worktreeDir: config.worktreeDir ?? join(homedir(), ".altcli"),
