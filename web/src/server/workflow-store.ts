...
export function wireText(input: StartInput, session: ManagedSession): string {
  ...
  return singleLine(`${text} [codercrew-command:${input.requestId}]`);
}
...
export class WorkflowStore {
  ...
  pause(id: string): void {
    this.store.db.transaction(() => { const run = this.run(id); if (!run) throw new AppError('NOT_FOUND', 'Run not found.', 404);
      ...
      run.pauseRequested = true; this.stop(run, 'Paused by the user. This does not interrupt an agent or release ownership.');
    }).immediate();
  }
