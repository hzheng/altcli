...
test('a unique marker distinguishes identical relay instructions', () => {
  assert.equal(marker(`relay: [altcli-command:${ID}]`), ID);
  assert.equal(marker(`first line\nsecond line [altcli-command:${ID}]`), ID);
  assert.equal(marker('relay'), undefined);
  assert.equal(marker(`relay: [altcli-command:${ID}] altered`), undefined);
});
...
test('Codex completion uses the exact native turn even when notify contains many historical commands', () => {
  const old = 'Old command [altcli-command:22345678-1234-4234-8234-123456789abc]';
  const prompt = `Finish this task. [altcli-command:${ID}]`;
...
test('Codex steering preserves the native binding but conflicting commands and later unmarked turns do not borrow it', () => {
  ...
  const conflict = codexStart('Other [altcli-command:22345678-1234-4234-8234-123456789abc]', 'turn1', first);
...
test('Codex native hook installation preserves unrelated hooks and is idempotent', () => {
  const input = { hooks: { Interrupt: [{ hooks: [{ type: 'command', command: 'foreign-interrupt' }] }], Stop: [{ hooks: [{ type: 'command', command: 'foreign-stop' }] }], UserPromptSubmit: [{ hooks: [
    { type: 'command', command: '/old/altcli-turn-complete.sh codex-start' }, { type: 'command', command: 'foreign-start' }] }] }, custom: true };
  const next = codexHooks(JSON.stringify(input), '/new/altcli-turn-complete.sh'); const parsed = JSON.parse(next);
...
test('Claude installation preserves unrelated hooks in the same group', () => {
  const input = { hooks: { Stop: [{ matcher: '*', hooks: [{ type: 'command', command: '/old/altcli-turn-complete.sh claude' }, { type: 'command', command: 'foreign-hook' }] }] }, custom: { keep: true } };
  const next = JSON.parse(claudeSettings(JSON.stringify(input), '/new/altcli-turn-complete.sh'));
  ...
  assert.equal(claudeSettings(JSON.stringify(next), '/new/altcli-turn-complete.sh'), JSON.stringify(next, null, 2) + '\n');
});
test('multiline TOML string arrays and trailing comments are replaced as one value', () => {
  ...
  const output = codexConfig(original, '/new/altcli-turn-complete.sh');
  assert.match(output, /"--then","notify-send","existing notifier"/);
  assert.match(output, /\] # notify note/);
  assert.match(output, /\[profiles.test\]\nnotify = \["do-not-change"\]/);
  assert.equal(codexConfig(output, '/new/altcli-turn-complete.sh'), output);
});
...
test('the installer validates both files before writing either and creates backups on success', () => {
  const home = mkdtempSync(join(tmpdir(), 'altcli-config-'));
  try {
    ...
    assert.ok(readdirSync(claude).some((p) => p.includes('altcli-backup')));
    assert.ok(readdirSync(codex).some((p) => p.includes('altcli-backup')));
...
test('a native start requires its exact valid prompt_id on Stop, not a session-only downgrade', () => {
  const saved = claudeStartState({ session_id: 's1', prompt_id: 'p-current', prompt: `relay: [altcli-command:${ID}]` }, identity, null);
...
test('a malformed present start ID is not silently treated as an older CLI', () => {
  for (const value of [null, '', 7, 'bad\nid', 'x'.repeat(201)]) {
    const payload = { session_id: 's1', prompt_id: value, prompt: `relay: [altcli-command:${ID}]` };
...
test('legacy pairing requires both events to omit prompt_id, even if a supplied ID equals the generated UUID', () => {
  const payload = { session_id: 's1', prompt: `relay: [altcli-command:${ID}]` };
  const saved = claudeStartState(payload, identity, null);
...
test('a fresh native ID recovers an interrupted legacy slot without accepting the old Stop', () => {
  ...
  const saved = claudeStartState({ session_id: 's1', prompt_id: 'p-new', prompt: `relay: [altcli-command:${ID}]` }, identity, old);
...
test('unversioned on-disk slots do not guess whether a UUID was native or generated', () => {
  const start = claudeStart({ session_id: 's1', prompt_id: ID, prompt: `relay: [altcli-command:${ID}]` }, identity, null);
...
test('pairing provenance survives JSON storage but stays out of the HTTP event context', () => {
  const saved = JSON.parse(JSON.stringify(claudeStartState({ session_id: 's1', prompt_id: 'p-new', prompt: `relay: [altcli-command:${ID}]` }, identity, null)));
...
test('a task notification is not a turn start; the Stop closing it still completes the active command', () => {
  ...
  const saved = claudeStartState({ session_id: 's1', prompt_id: 'p-cmd', prompt: `relay: [altcli-command:${ID}]` }, identity, null);
  ...
  const legacy = claudeStartState({ session_id: 's1', prompt: `relay: [altcli-command:${ID}]` }, identity, null);
...
test('the real hooks preserve Claude prompt pairing and Codex correlation through steering', async () => {
  ...
  try {
    ...
    const env = { ...process.env, HOME: home, PATH: `${bin}:${process.env.PATH ?? ''}`, TMUX_PANE: identity.paneId,
      TMUX: `${identity.socketPath},${identity.serverPid},0`, ALTCLI_ENV: envFile, ALTCLI_URL: `http://127.0.0.1:${server.address().port}` };
    ...
    await invoke({ hook_event_name: 'UserPromptSubmit', prompt_id: 'p-current', prompt: `relay: [altcli-command:${ID}]` });
    assert.equal(events.length, 2); assert.equal(events[1].commandId, ID);
    const slot = join(home, '.local', 'share', 'altcli', 'hook-turns', `${contextKey(identity, 's1')}.json`);
    ...
    const prompt = `Finish the task. [altcli-command:${ID}]`;
    ...
    const codex = { type: 'agent-turn-complete', 'thread-id': 'codex-session', 'turn-id': 'codex-turn',
      'input-messages': ['Earlier [altcli-command:22345678-1234-4234-8234-123456789abc]', prompt, 'One more clarification.'], 'last-assistant-message': 'Finished.' };
    ...
    const binding = readFileSync(join(home, '.local', 'share', 'altcli', 'hook-turns', `${contextKey(identity, 'codex-session')}.codex.json`), 'utf8');
    ...
    assert.equal(readFileSync(join(home, '.local', 'share', 'altcli', 'hook-turns', `${contextKey(identity, 'codex-session')}.codex.json`), 'utf8'), binding);
    ...
    assert.doesNotMatch(diagnostic, /desktop text|Current response|altcli-command|aaaaaaa/);
    ...
    const codexSlot = join(home, '.local', 'share', 'altcli', 'hook-turns', `${contextKey(identity, 'codex-session')}.codex.json`);
