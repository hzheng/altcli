...
const roots = [join(homedir(), ".claude", "skills"), join(process.env.CODEX_HOME ?? join(homedir(), ".codex"), "skills")];
...
if (check && !(await exists(join(homedir(), ".claude"))) && !(await exists(join(process.env.CODEX_HOME ?? join(homedir(), ".codex"))))) {
